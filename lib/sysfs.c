...
/*
 * Converts device name to devno. @prefix allows to redirect paths to another
 * location (e.g. /mytests/sys/ ...)
 */
static dev_t __sysfs_devname_to_devno(const char *prefix, const char *name, const char *parent)
{
	...
	if (!dev && parent && strncmp("dm-", name, 3)) {
		...
		len = snprintf(buf, sizeof(buf), "%s%s/%s/%s/dev",
				prefix,	_PATH_SYS_BLOCK, _parent, _name);
		free(_name);
		free(_parent);
		...
		len = snprintf(buf, sizeof(buf), "%s%s/%s/dev",
				prefix, _PATH_SYS_BLOCK, _name);
		free(_name);
		if (len < 0 || (size_t) len >= sizeof(buf))
			return 0;
		path = buf;
